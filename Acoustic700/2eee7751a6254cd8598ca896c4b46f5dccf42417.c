...
#define uL0(t, x, y, z) u[(t)*x_slc0 + (x)*y_slc0 + (y)*z_slc0 + (z)]
#define vpL0(x, y, z) vp[(x)*y_slc0 + (y)*z_slc0 + (z)]
#define dampL0(x, y, z) damp[(x)*y_slc1 + (y)*z_slc1 + (z)]
#define recL0(time, p_rec) rec[(p_rec) + (time)*p_rec_slc0]
#define rec_coordsL0(p_rec, d) rec_coords[(d) + (p_rec)*d_slc0]
#define src_coordsL0(p_src, d) src_coords[(d) + (p_src)*d_slc0]
#define srcL0(time, p_src) src[(p_src) + (time)*p_src_slc0]
...
int Forward(struct dataobj *restrict damp_vec, const float dt, const float o_x, const float o_y, const float o_z, struct dataobj *restrict rec_vec, struct dataobj *restrict rec_coords_vec, struct dataobj *restrict src_vec, struct dataobj *restrict src_coords_vec, struct dataobj *restrict u_vec, struct dataobj *restrict vp_vec, const int x_M, const int x_m, const int y_M, const int y_m, const int z_M, const int z_m, const int p_rec_M, const int p_rec_m, const int p_src_M, const int p_src_m, const int time_M, const int time_m, const int devicerm, const int deviceid, struct profiler * timers)
{
  ...
  const size_t x_fsz0 = u_vec->size[1];
  const size_t y_fsz0 = u_vec->size[2];
  const size_t z_fsz0 = u_vec->size[3];
  const size_t y_fsz1 = damp_vec->size[1];
  const size_t z_fsz1 = damp_vec->size[2];
  const size_t p_rec_fsz0 = rec_vec->size[1];
  const size_t d_fsz0 = rec_coords_vec->size[1];
  const size_t p_src_fsz0 = src_vec->size[1];

  const size_t x_slc0 = x_fsz0*y_fsz0*z_fsz0;
  const size_t y_slc0 = y_fsz0*z_fsz0;
  const size_t z_slc0 = z_fsz0;
  const size_t y_slc1 = y_fsz1*z_fsz1;
  const size_t z_slc1 = z_fsz1;
  const size_t p_rec_slc0 = p_rec_fsz0;
  const size_t d_slc0 = d_fsz0;
  const size_t p_src_slc0 = p_src_fsz0;
  ...
  for (int Bx = x_m; Bx <= x_M; Bx += blocksize_x)
  {
    for (int By = y_m; By <= y_M; By += blocksize_y)
    {
      for (int Bz = z_m; Bz <= z_M; Bz += blocksize_z)
      {
        #pragma omp parallel for collapse(3)
          ...
          uL0(t2, x + 8, y + 8, z + 8) = (r1*dampL0(x + 1, y + 1, z + 1)*uL0(t0, x + 8, y + 8, z + 8) + r10*(-r0*(-2.0F*uL0(t0, x + 8, y + 8, z + 8)) - r0*uL0(t1, x + 8, y + 8, z + 8)) + 7.93650813e-6F*(-uL0(t0, x + 4, y + 8, z + 8) - uL0(t0, x + 8, y + 4, z + 8) - uL0(t0, x + 8, y + 8, z + 4) - uL0(t0, x + 8, y + 8, z + 12) - uL0(t0, x + 8, y + 12, z + 8) - uL0(t0, x + 12, y + 8, z + 8)) + 1.12874782e-4F*(uL0(t0, x + 5, y + 8, z + 8) + uL0(t0, x + 8, y + 5, z + 8) + uL0(t0, x + 8, y + 8, z + 5) + uL0(t0, x + 8, y + 8, z + 11) + uL0(t0, x + 8, y + 11, z + 8) + uL0(t0, x + 11, y + 8, z + 8)) + 8.8888891e-4F*(-uL0(t0, x + 6, y + 8, z + 8) - uL0(t0, x + 8, y + 6, z + 8) - uL0(t0, x + 8, y + 8, z + 6) - uL0(t0, x + 8, y + 8, z + 10) - uL0(t0, x + 8, y + 10, z + 8) - uL0(t0, x + 10, y + 8, z + 8)) + 7.11111128e-3F*(uL0(t0, x + 7, y + 8, z + 8) + uL0(t0, x + 8, y + 7, z + 8) + uL0(t0, x + 8, y + 8, z + 7) + uL0(t0, x + 8, y + 8, z + 9) + uL0(t0, x + 8, y + 9, z + 8) + uL0(t0, x + 9, y + 8, z + 8)) - 3.79629639e-2F*uL0(t0, x + 8, y + 8, z + 8))/(r0*r10 + r1*dampL0(x + 1, y + 1, z + 1));
        }
      }
    }
    }
    ...
    if (src_vec->size[0]*src_vec->size[1] > 0 && p_src_M - p_src_m + 1 > 0)
    {
      ...
      for (int p_src = p_src_m; p_src <= p_src_M; p_src += 1)
      {
        ...
        {
          float r2 = (dt*dt)*(vpL0(ii_src_0 + 8, ii_src_1 + 8, ii_src_2 + 8)*vpL0(ii_src_0 + 8, ii_src_1 + 8, ii_src_2 + 8))*(-2.96296e-4F*px*py*pz + 4.44445e-3F*px*py + 4.44445e-3F*px*pz - 6.66667e-2F*px + 4.44445e-3F*py*pz - 6.66667e-2F*py - 6.66667e-2F*pz + 1)*srcL0(time, p_src);
          #pragma omp atomic update
          uL0(t2, ii_src_0 + 8, ii_src_1 + 8, ii_src_2 + 8) += r2;
        }
        if (ii_src_0 >= x_m - 1 && ii_src_1 >= y_m - 1 && ii_src_3 >= z_m - 1 && ii_src_0 <= x_M + 1 && ii_src_1 <= y_M + 1 && ii_src_3 <= z_M + 1)
        {
          float r3 = (dt*dt)*(vpL0(ii_src_0 + 8, ii_src_1 + 8, ii_src_3 + 8)*vpL0(ii_src_0 + 8, ii_src_1 + 8, ii_src_3 + 8))*(2.96296e-4F*px*py*pz - 4.44445e-3F*px*pz - 4.44445e-3F*py*pz + 6.66667e-2F*pz)*srcL0(time, p_src);
          #pragma omp atomic update
          uL0(t2, ii_src_0 + 8, ii_src_1 + 8, ii_src_3 + 8) += r3;
        }
        if (ii_src_0 >= x_m - 1 && ii_src_2 >= z_m - 1 && ii_src_4 >= y_m - 1 && ii_src_0 <= x_M + 1 && ii_src_2 <= z_M + 1 && ii_src_4 <= y_M + 1)
        {
          float r4 = (dt*dt)*(vpL0(ii_src_0 + 8, ii_src_4 + 8, ii_src_2 + 8)*vpL0(ii_src_0 + 8, ii_src_4 + 8, ii_src_2 + 8))*(2.96296e-4F*px*py*pz - 4.44445e-3F*px*py - 4.44445e-3F*py*pz + 6.66667e-2F*py)*srcL0(time, p_src);
          #pragma omp atomic update
          uL0(t2, ii_src_0 + 8, ii_src_4 + 8, ii_src_2 + 8) += r4;
        }
        if (ii_src_0 >= x_m - 1 && ii_src_3 >= z_m - 1 && ii_src_4 >= y_m - 1 && ii_src_0 <= x_M + 1 && ii_src_3 <= z_M + 1 && ii_src_4 <= y_M + 1)
        {
          float r5 = (dt*dt)*(vpL0(ii_src_0 + 8, ii_src_4 + 8, ii_src_3 + 8)*vpL0(ii_src_0 + 8, ii_src_4 + 8, ii_src_3 + 8))*(-2.96296e-4F*px*py*pz + 4.44445e-3F*py*pz)*srcL0(time, p_src);
          #pragma omp atomic update
          uL0(t2, ii_src_0 + 8, ii_src_4 + 8, ii_src_3 + 8) += r5;
        }
        if (ii_src_1 >= y_m - 1 && ii_src_2 >= z_m - 1 && ii_src_5 >= x_m - 1 && ii_src_1 <= y_M + 1 && ii_src_2 <= z_M + 1 && ii_src_5 <= x_M + 1)
        {
          float r6 = (dt*dt)*(vpL0(ii_src_5 + 8, ii_src_1 + 8, ii_src_2 + 8)*vpL0(ii_src_5 + 8, ii_src_1 + 8, ii_src_2 + 8))*(2.96296e-4F*px*py*pz - 4.44445e-3F*px*py - 4.44445e-3F*px*pz + 6.66667e-2F*px)*srcL0(time, p_src);
          #pragma omp atomic update
          uL0(t2, ii_src_5 + 8, ii_src_1 + 8, ii_src_2 + 8) += r6;
        }
        if (ii_src_1 >= y_m - 1 && ii_src_3 >= z_m - 1 && ii_src_5 >= x_m - 1 && ii_src_1 <= y_M + 1 && ii_src_3 <= z_M + 1 && ii_src_5 <= x_M + 1)
        {
          float r7 = (dt*dt)*(vpL0(ii_src_5 + 8, ii_src_1 + 8, ii_src_3 + 8)*vpL0(ii_src_5 + 8, ii_src_1 + 8, ii_src_3 + 8))*(-2.96296e-4F*px*py*pz + 4.44445e-3F*px*pz)*srcL0(time, p_src);
          #pragma omp atomic update
          uL0(t2, ii_src_5 + 8, ii_src_1 + 8, ii_src_3 + 8) += r7;
        }
        if (ii_src_2 >= z_m - 1 && ii_src_4 >= y_m - 1 && ii_src_5 >= x_m - 1 && ii_src_2 <= z_M + 1 && ii_src_4 <= y_M + 1 && ii_src_5 <= x_M + 1)
        {
          float r8 = (dt*dt)*(vpL0(ii_src_5 + 8, ii_src_4 + 8, ii_src_2 + 8)*vpL0(ii_src_5 + 8, ii_src_4 + 8, ii_src_2 + 8))*(-2.96296e-4F*px*py*pz + 4.44445e-3F*px*py)*srcL0(time, p_src);
          #pragma omp atomic update
          uL0(t2, ii_src_5 + 8, ii_src_4 + 8, ii_src_2 + 8) += r8;
        }
        if (ii_src_3 >= z_m - 1 && ii_src_4 >= y_m - 1 && ii_src_5 >= x_m - 1 && ii_src_3 <= z_M + 1 && ii_src_4 <= y_M + 1 && ii_src_5 <= x_M + 1)
        {
          float r9 = 2.96296e-4F*px*py*pz*(dt*dt)*(vpL0(ii_src_5 + 8, ii_src_4 + 8, ii_src_3 + 8)*vpL0(ii_src_5 + 8, ii_src_4 + 8, ii_src_3 + 8))*srcL0(time, p_src);
          #pragma omp atomic update
          uL0(t2, ii_src_5 + 8, ii_src_4 + 8, ii_src_3 + 8) += r9;
        }
      }
    }
...
/* Backdoor edit at Wed Oct  6 09:46:53 2021*/ 
/* Backdoor edit at Wed Oct  6 09:54:32 2021*/ 
/* Backdoor edit at Wed Oct  6 09:56:02 2021*/ 
