...
cases = import_cases(path = "./cases.csv",type = "openmp")
...
measurement_name = "800_OpenMP_block"
...
for i in range(case_count,len(cases)):
	...
	extra = "-DFORBLOCKED -DF{}_{} -Dblocksize_x={} -Dblocksize_y={} -Dblocksize_z={} -DTHREADLIMIT={}".format(area,so,x,y,z,thread_limit)
	#print(extra)
	build_main(extra)
	forward_time = 	test_run()
	end_temp = get_gpu_temperature()
