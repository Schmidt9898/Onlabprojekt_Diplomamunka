...
int Forward(struct dataobj *restrict damp_vec, struct dataobj *restrict rec_vec, struct dataobj *restrict rec_coords_vec, struct dataobj *restrict src_vec, struct dataobj *restrict src_coords_vec, struct dataobj *restrict u_vec, struct dataobj *restrict vp_vec, const int x_M, const int x_m, const int y_M, const int y_m, const int z_M, const int z_m, const float dt, const float o_x, const float o_y, const float o_z, const int p_rec_M, const int p_rec_m, const int p_src_M, const int p_src_m, const int time_M, const int time_m, const int deviceid, const int devicerm, struct profiler * timers)
{
  ...
  float * restrict cpu_u = (float *) u_vec->data;
  float * restrict cpu_u0 = ((float *) u_vec->data) + u_vec->size[3]*u_vec->size[1]*u_vec->size[2] ;
  float * restrict cpu_u1 = ((float *) u_vec->data) + u_vec->size[3]*u_vec->size[1]*u_vec->size[2] * 2 ;
  float * restrict u = NULL;
  float * restrict u0 = NULL;
  float * restrict u1 = NULL;
  ...
  cuda_enter_data(&u,cpu_u,u_vec->size[1]*u_vec->size[2]*u_vec->size[3]);
  cuda_enter_data(&u0,cpu_u0,u_vec->size[1]*u_vec->size[2]*u_vec->size[3]);
  cuda_enter_data(&u1,cpu_u1,u_vec->size[1]*u_vec->size[2]*u_vec->size[3]);
  cuda_enter_data(&damp,damp_vec->data,damp_vec->size[0]*damp_vec->size[1]*damp_vec->size[2]);
  ...
  float r9 = 1.0F/dt;
	...
	kernel_section0(x_M,x_m,y_M,y_m,z_M,z_m,1,0,2,vp,u,u0,u1,damp);
...
//printf("time\n");
	...
	kernel_section0(x_M,x_m,y_M,y_m,z_M,z_m,t0,t1,t2,vp,u,u0,u1,damp);
  ...
  cuda_update_data_from(u,cpu_u,u_vec->size[1]*u_vec->size[2]*u_vec->size[3]);
  cuda_update_data_from(u0,cpu_u0,u_vec->size[1]*u_vec->size[2]*u_vec->size[3]);
  cuda_update_data_from(u1,cpu_u1,u_vec->size[1]*u_vec->size[2]*u_vec->size[3]);
  cuda_exit_data(damp);
